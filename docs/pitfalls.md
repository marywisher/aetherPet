...
### [2026-09-15] 发布前增量 1：P0-1「创建当日馈赠跳过」误用 dailyGrantLastDate → 馈赠永久停摆，判据必须是 pet.created_at

**阶段**：发布前增量 1（pre-launch）

**问题描述**：
需求「pet 创建当日跳过馈赠（首馈赠在 T+1）」。若用 `dailyGrantLastDate === today` 判断「今日已发放」来兜底：创建当日 pet 没有任何历史馈赠记录，该字段为空、判定不命中，当日馈赠照常发生（跳过失效）；若再「顺手」在创建时把该字段写成今天，则后续每次发放前判断恒为「今日已发放」，馈赠**永久停摆**。两条路都错。

**根因分析**：
混淆了两类「今天」：`dailyGrantLastDate` 只记录**历史发放事实**（状态字段），而「创建日」是**生命周期锚点**，不是一次历史发放。用状态字段去表达生命周期语义，天然错位。

**修复方案**：
`src/domain/gift/daily-grant.ts` 的跳过判据改为 `pet.createdAt`（created_today → 跳过），DB 侧幂等条件仍走 `dailyGrantLastDate` 列各司其职；单测把 pet fixture 的 createdAt 提前 >24h 避开 created_today 误命中；`docs/acceptance-runbook.md` 验收 10 口径同步更新为「首馈赠在 T+1」。

**预防措施**：
写「第 X 天跳过/执行」类逻辑前先问：这一天是不是「可以被写入的状态」？生命周期锚点（created_at）与状态字段（last_x_date）不可互替；改动馈赠/幂等类逻辑必须补「创建后第一天」的回归用例。
...
### [2026-09-15] 发布前增量 1：pack_schema 1.1.0 新增可选键用「loader 字段级物化」，不做渲染时 merge（语义污染防线）

**阶段**：发布前增量 1（pre-launch）

**问题描述**：
1.1.0 新增可选键 `texts.first_meeting` / `texts.guidance` / `images.first_meeting_note`。若「缺键→取 default 包」的回退放在**渲染时**（模板层现算），则非 default 包缺失字段会静默混入 default 包内容——且填充来源随 default 包演进漂移，同一 pack 在不同时间渲染结果不同；同时 1.0.0 的 11 个必填键仍必须走整包级回退（`fallback.ts`），两套机制混在同一层会把「整包不完整替身」与「单字段补齐」的语义搅在一起。

**根因分析**：
「字段级回退」与「整包回退」是两个一致性域：整包回退=该 pack 整体不完整、用整包替身；可选键回退=该字段本包不提供、由 default 包在**加载时**补齐。加载时物化（写进 pack 对象）后，后续所有层看到的都是「本包有该键」，回退结果被固化，语义不扩散。

**修复方案**：
`src/domain/packs/loader.ts` 加载时对 1.1.0 新增可选键做字段级物化：非 default 包缺键即从 default 包同名键拷贝（旧 1.0.x 包零改动可用）；边界显式注释——仅物化新增可选键，11 个必填键仍走原整包回退。

**预防措施**：
契约新增可选键时，第一问就是「谁补、在哪层补」（loader 物化 vs 渲染时 merge vs 改为必填），并把选定层写进契约文档；选定 loader 物化后，任何 pack 的回退结果在加载后即固化，不受 default 包后续演进影响。
...
### [2026-09-15] 发布前增量 1：初始事件倒推允许 ts 早于 pet.created_at——领域语义没进 CONTEXT.md，QA 会误判成数据异常

**阶段**：发布前增量 1（pre-launch）

**问题描述**：
创建事务倒推 1–2 条「初始事件」（watching_water / counting_leaves / self_talk），ts 刻意早于创建时刻（叙事：它在你见到它之前就已经在过日子）。DB 无「event ts ≥ pet.created_at」约束，代码自洽；但只按「事件时间不该早于 pet 创建」直觉读验收材料的 QA，会把倒推 ts 判成数据异常并开出 P 级缺陷。

**根因分析**：
「倒推是 by design」这一隐含领域语义只在代码注释里，未登记进术语表/领域文档。跨角色（开发→质检）传递时，注释不可见，只能靠词汇表兜底。

**修复方案**：
`CONTEXT.md` 显式补记：「倒推初始事件的 ts 可早于 pet.created_at，属设计语义，无约束」。QA 验收口径以 CONTEXT.md 为准。

**预防措施**：
凡是「故意违反常识约束」的设计（ts 顺序、唯一性、数值区间），必须同步在 CONTEXT.md 加一行领域语义——代码注释防不住跨角色误判，词汇表可以。
...
### [2026-09-15] 发布前增量 1：engine typePool 是「新增可选参数」而非默认替换——否则静默改变验收 3 随机分布

**阶段**：发布前增量 1（pre-launch）

**问题描述**：
为支持「初见」等固定事件，引擎 `generateBatchEvents` 增加 `typePool` 限定生成池。若把新行为做成**默认路径**（旧调用点即使不传参也走 pool 分支），现有所有调用方在不传参时行为也可能变化——验收 3 的随机事件分布被**静默**改变，且既有测试全绿（无人对分布形状做统计断言），只有验收方统计口径才能发现漂移。

**根因分析**：
向后兼容扩展应是「加可选参数，旧路径不动」；默认替换则是对所有现存调用方的隐性契约变更。「不传参也变了行为」的静默改动，是单测结构上抓不到的（单测断言单条生成结果，不断言分布）。

**修复方案**：
`EngineOptions.typePool` 为可选参数：传了才限定池，不传走原默认随机路径，`RANDOM_TYPES` 保持 6 项不动；`tests/unit/domain/events/first-meeting-guards.test.ts` 用 4 道守卫锁边界——RANDOM_TYPES 不含 first_meeting、补算池不含、forceType 可生成、typePool 限定。

**预防措施**：
引擎类公共函数加参数，默认必须是「不传参=行为不变」；任何影响分布/概率/随机种的改动，配套加统计形状断言（分布型验收），不能只靠逐条生成单测。
