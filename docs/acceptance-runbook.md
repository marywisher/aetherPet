...
> ⚠️ **pre-launch 口径变更（P0-1）**：创建 pet 当日不发放每日馈赠（`skipReason='created_today'`），首馈赠自 T+1（创建次日）首次登录开始。
> 理由：首日由初见字条 + 倒推小日子承担欢迎仪式，馈赠卡会让位，保证「初见=首页第一条」；次日馈赠卡的「放桌上，看看它会怎么回应吧」接手指引。

**操作**
1. 创建 pet 当日登录 → 首页**不应出现**「今日馈赠」卡，只看到初见字条 + 倒推小事（时间线第一条 = `first_meeting`）
2. 次日（或改 `pets.created_at` 为 2 天前模拟）登录 → 首页出现「今日馈赠」→ 领取 → 获得随机物品（物品池 ≥8 种）
3. 把物品送给宠物（/gifts）→ 宠物 24h 内回信（/letter，含 ≥1 条记忆引用）
4. 同一设备/账号当天**再次**尝试领取 → 应被日限拒绝（每天一次）
5. 退避验证：把 `pets.user_last_active_ts` 改为 3 天前 → 刷新，观察 pet 主动触达（如「好久不见」事件），事件间隔参数应显示为退避 7 天
